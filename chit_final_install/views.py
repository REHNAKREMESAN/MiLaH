...
def chitfinalinstallment(request, pk):
    ...
    obj_memb = LabReg.objects.get(id=mid)
    #membname = obj_memb.member_name

    labourid = LabReg.objects.filter(id=memberid.members_name_id)
    ...
    if request.method == 'POST':
        ...
        try:
            fdinstalobj = ChitFinalInstallment.objects.get(fchit_name_id=obj.id, fmember_name=mid)
            ...
            chit_draw_obj = ChitFinalDraw.objects.get(fmember_name=memberid.members_name_id)
            ...
            labid=memberid.members_name_id
            c = ChitFinalInstallment(fchit_name_id=obj.id, fmember_name=labid,
